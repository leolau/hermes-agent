...
**Wave:** B→C (Phase-2) · **Owner agent:** _unassigned_ · **Status:** FG-17a landed; FG-17b implemented (code + tests green) — ECS system test is the remaining Leo-owned gate
...
**FG-17b — new panels:**
- [x] Core-area view (FG-14 manifest/health + FG-12 change log + FG-16 trace) — read-only `GET /api/core/manifest` projection + `web/src/screens/CorePage.tsx` (boundary health, active globs, self-protection, denials, change log, trace)
- [x] Embedded Telegram chat → same FG-03 one-brain backend — `web/src/screens/TelegramPage.tsx` surfaces the live Telegram channel status (reusing `/api/messaging/platforms`) and routes the in-dashboard conversation to the existing one-brain chat (`/chat` → TUI PTY → `tui_gateway` → `AIAgent`), which is the same backend the Telegram app talks to. **Decision (recorded per §Implementation step 3):** the official Telegram web-widget cannot be embedded under the dashboard's auth/security model, so the doc-sanctioned **native-chat fallback** is used — the shared one-brain chat pane, not a second chat/agent implementation.
- [x] Agent webview (CDP) — consent-gated (C6) + traced (C8) + per-user profile isolation — `hermes_cli/webview.py` policy (default-deny; read-only vs interactive; credentialed/destructive/off-scope escalation) + `web_server.py` `/api/webview/*` endpoints (open/get/close/action/approval) over the existing `tools/browser_cdp_tool` CDP toolset, per-user opaque profile dirs (UUID5), C8 `InteractionLedger` tracing; `web/src/screens/WebviewPage.tsx` consent + action + approval UI
- [x] Tool link/icon registration (FG-07 registry; Next.js+Node tools) + dev→prod promote — **reused as-is**: FG-07 tools that ship a dashboard manifest already register a link/icon/route via the existing dashboard-plugin manifest system (`usePlugins()`/`PluginPage` in `web/src/App.tsx`, `has_dashboard_manifest` + enable/disable/configure/promote/health on the Plugins page). No new registration surface added (Footprint Ladder rung 1 — extend existing).
- [x] tests (17b unit + negative + E2E) green — `tests/hermes_cli/test_webview.py` (8, policy/isolation/profile-path) + `tests/hermes_cli/test_fg17b_dashboard_e2e.py` (7, real FastAPI + Postgres: default-deny, allow+escalate, approval grant/deny, C8 traces, per-user session isolation, Core projection, GTS graph shape/scope + FG-19 assignment/grants); web vitest for the webview/telegram helpers (`web/src/lib/webview.test.ts`, `telegram.test.ts`)
- [ ] System test on the system-test ECS passed (see *System testing* section) — **Leo-owned gated step; not run by this session**
...
| 2026-07-04 | 4 | devin (for Leo) | **FG-17b panels implemented on `develop` (rebased on merged FG-19).** Backend: `/api/core/manifest` (read-only C7 projection), `/api/gts/graph` now exposes merged FG-19 per-user assignment (`assignee_user_id` + per-item `grants` per node, `assignment={enabled,scheme:"per-user"}`), and `/api/webview/*` (open/get/close/action/approval) over the existing CDP toolset with default-deny + C6 escalation + C8 tracing + per-user UUID5 profile dirs (`hermes_cli/webview.py`); fixed a `NameError: uuid` in the escalation path. Frontend: `CorePage`, `GtsCentrePage` (now renders assignee/watcher badges), `OnboardingPage`, new `WebviewPage` + `TelegramPage`, webview API client + types, `web/src/lib/{webview,telegram}.ts` helpers, nav/routes wired in `App.tsx`. **Telegram decision:** native-chat fallback (reuse the one-brain `/chat`) since the official web-widget can't be embedded under dashboard auth. **Tool link/icon registration:** reused the existing dashboard-plugin manifest system (no new surface). Tests: `test_webview.py` (8) + `test_fg17b_dashboard_e2e.py` (7, real FastAPI+Postgres) + web vitest helpers; web lint(0 err)/typecheck/build green; `ruff` clean; FG-19 E2E still green. **NOT run:** ECS system test + prod promotion (Leo-owned gates). | FG-17b delivery: land the new panels + webview on top of merged FG-19, tested on real paths, without a backend rewrite or a second chat/agent implementation |
