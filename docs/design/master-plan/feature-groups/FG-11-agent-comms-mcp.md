...
## Definition of Done
Tests green + baseline green + `ruff`/`ty` clean; `mcp_serve.py` extended (not forked); MCP surface documented as a contract; cache-safety preserved (no live toolset splicing).
