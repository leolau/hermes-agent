...
## Definition of Done
Tests green (incl. concurrency + negative access + cache-safety) + baseline green + `ruff`/`ty` clean; pgvector provider registered via the memory-provider ABC; curated snapshot semantics untouched.
