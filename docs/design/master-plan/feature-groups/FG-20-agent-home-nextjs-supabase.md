...
**Routes:** `/login` (bridge login) · `/` (seam proof: principal + one RLS-scoped read; links to `/core`) · `/graph` (Wave B1: read-only GTS Centre) · `/activity` (Wave B2: C8 interaction-trace timeline) · `/activity/[traceId]` (Wave B2: one trace's event timeline) · `/core` (Wave B2: read-only Core-area view) · `/onboarding` (Wave B3: readiness wizard) · `/tools` (Wave B3: read-only tool registry) · `/chat` (tab placeholder) · `POST /api/session/login` · `POST /api/session/logout`.
